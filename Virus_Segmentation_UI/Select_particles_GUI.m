function varargout = Select_particles_GUI(varargin)
...
if max(size(TableData)) > max(size(Objects))
    TableData(max(size(Objects)+1):end) = [];
end

Objects_out = Objects;
Objects_out(logical(TableData)) = [];

disp('Output')
varargout = {Objects_out}
