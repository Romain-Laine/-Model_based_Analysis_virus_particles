function [ H_mean, H_std, Envelope, CI_upperbound, CI_lowerbound ] = RipleyK_CIcalc_dualColor( Fov, Density1, Density2, Analysis_window, r_step, N_repeat, p )
...
disp(['Density of population 1 used to calculate CI: ',num2str(10^6*Density1), ' centroids / um^2']);
disp(['Density of population 2 used to calculate CI: ',num2str(10^6*Density2), ' centroids / um^2']);

av_N_loc1 = ceil(Density1*Fov(1)*Fov(2));
disp(['Number of localizations used to calculate CI: ',num2str(av_N_loc1)]);
av_N_loc2 = ceil(Density2*Fov(1)*Fov(2));
disp(['Number of localizations used to calculate CI: ',num2str(av_N_loc2)]);
...
N_loc1 = poissrnd(av_N_loc1,N_repeat,1);
N_loc2 = poissrnd(av_N_loc2,N_repeat,1);


for i = 1:N_repeat
    [ X1,Y1 ] = Generate_random_XY( Fov, N_loc1(i) );
    [ X2,Y2 ] = Generate_random_XY( Fov, N_loc2(i) );
    [ ~, ~, K_all(:,i), ~ ] = CalcRipleyK( X2, Y2, X1, Y1, Fov, Fov(1)*Fov(2), Analysis_window, r_step );
